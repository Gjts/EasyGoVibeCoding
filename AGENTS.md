...
This is a **documentation-driven** education platform. The original roadmap had 8 core modules, and the current implementation has expanded beyond that set:
...
9. **超级个体篇 (Super Individual)** - Productizing and scaling individual capability
10. **GitHub 热门项目** - AI/project ecosystem radar
11. **日本站 MVP (`/ja`)** - Japanese landing, waitlist, and legal pages

### Key Architecture Patterns

**Static Site Generation (SSG) First**
- All content pages use SSG for optimal performance and SEO
- Current content is implemented primarily as `app/**/page.tsx` pages.
- The earlier MDX/content plan is not implemented yet; do not assume `content/`, `lib/mdx.ts`, or `lib/content.ts` exist.
...
**Data Files**
- `data/models.json` - Static fallback seed for the latest-models panel
- Tool/provider/MCP JSON files from the early roadmap are not currently present

**Utility Libraries (`lib/`)**
- `lib/utils.ts` - General utility functions (cn() for className merging)
- `lib/models.ts` and `lib/model-schema.ts` - model data fetch, fallback, and validation
- `lib/learning-progress.ts` - local visit-based learning progress

**Custom Hooks (`hooks/`)**
- `use-mobile.tsx` - Mobile device detection (from shadcn/ui)

**Build Scripts (`scripts/`)**
- `build-with-meta.js` - Next static build with metadata handling
- `copy-functions.js` - Copy Cloudflare Pages Functions into `out/functions`
- `validate-models-json.mjs` - Validate `data/models.json`
...
### 1. Content as Code
- Current learning content is stored mostly as TSX pages under `app/`
- MDX/frontmatter remains a future option, not the current implementation
...
Current implemented/partial interactive surfaces:
- **SearchDialog** - Header search dialog with keyboard shortcut
- **LatestModelsPanel** - Official-source model release panel with API/seed fallback
- **LearningDashboard / VisitTracker** - Local visit-based learning dashboard
- **ModelTierList** - Model ranking display using `data/models.json`

Roadmap items not yet fully implemented:
- **ToolComparison** - Dynamic tool comparison matrix with filtering
- **SelectionAssistant** - Full question-based tool/architecture selection helper
- **LearningPathRecommender** - Full role-based learning path recommendation
...
Current content updates should usually be made in `app/**/page.tsx` files and related `components/**` files. The MDX/frontmatter guidance below is retained as a future content-system plan, not the current implementation.
...
- Future MDX content would be loaded from a `content/` directory after the pipeline exists
...
- Multi-module course pages across basics, advanced, tools, architecture, practice, team, ecosystem, resources, and super-individual
...
- Latest models panel with `/api/models` and static seed fallback
- GitHub hot projects page
- Japanese MVP landing, waitlist, and legal pages
- Local visit-based learning dashboard

**In Progress**:
- Content refinement across existing modules
- Interactive capability expansion beyond the current search experience
- Model-data automation and official-source verification

**Pending**:
- Account-based learning progress tracking
- Community features (comments, Q&A)
- Test framework and CI/CD workflow
...
5. **Add Content**
   - Add or modify `app/**/page.tsx` content first, matching nearby route patterns
   - Use MDX/frontmatter only after the MDX pipeline is actually implemented
...
2. **Content Updates**
   - Add/modify TSX pages under `app/` and shared content components under `components/`
   - Follow existing route/page patterns and keep headings semantically ordered
   - Do not assume a `content/` MDX directory exists
...
6. **Lint Is Not Yet Clean**
   - `pnpm lint` currently reports many issues
   - **Action Required**: fix unescaped entities, unused imports, render-time randomness, and script style issues before treating the app as production-clean

7. **No CI/CD Workflow**
   - `.github` workflows are not currently present
   - **Action Required**: add build/typecheck/lint/model validation workflow when release discipline matters

8. **sitemap/robots Missing**
   - Build output currently does not include `out/sitemap.xml` or `out/robots.txt`
   - **Action Required**: add generation before SEO-sensitive deployment
