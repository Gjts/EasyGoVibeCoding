...
export function buildUserPrompt(input: PromptInput): string {
  ...
  return [
    ...
    "- tier 必须按模型强度/综合能力分级，不得按发布时间或热度分级；免费、mini、lite、flash、small、XS 等轻量模型默认不能标为 tier 1，除非官方明确宣称它是当前旗舰/最高能力模型；首页当前明确保留 Gemini 3.5 Flash 为 tier 1 第四位",
    "- 第一梯队首页展示固定优先顺序：Anthropic、OpenAI、xAI、Google；xAI 当前代表为 Grok 4.5，Google 当前代表为 Gemini 3.5 Flash；实时发现官方新旗舰时可替换同厂商旧代表，但不要挤掉这四个旗舰位",
